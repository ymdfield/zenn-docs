...
[^11]: 厳密なツッコミを入れると、`IO`や`ST`に依存することで可変参照を利用することもでき、するとここでの議論が怪しくなるのですが、ここでの「状態が必要」を例えば「例外が必要」や「継続が必要」に読み替えて、リストの要素毎でバラバラの例外型`e`やanswer type `r`をもつ`ExceptT`や`ContT`が必要になるような例を考えるなどすることで同様の議論ができそうです。
